...
scripts/
  commit-msg                            Git commit-msg hook (strips AI branding)
templates/
...
- **No AI branding**: never include AI tool names, attribution, or advertising in commit messages (see `.cursor/rules/no-ai-branding-commits.mdc`)
- All tests must pass before merge
- Merge: `git checkout main && git merge feature/<name>`

### Git Hooks

A `commit-msg` hook in `scripts/commit-msg` automatically strips AI tool branding from commit messages. Install it after cloning:

```bash
cp scripts/commit-msg .git/hooks/commit-msg && chmod +x .git/hooks/commit-msg
```

The hook scans for AI tool names (Claude, Cursor, Copilot, ChatGPT, etc.), attribution phrases ("Generated by", "Created with", etc.), git trailers referencing AI tools, and marketing language ("AI-generated", "LLM-assisted", etc.). Matching lines are removed and a warning is printed. If the message is empty after stripping, the commit is rejected.
