"""Tests for nasa_apod.services.apod.config."""
...
class TestApodConfigConstants:
    ...
    def test_first_apod_date(self) -> None:
        from nasa_apod.services.apod.config import FIRST_APOD_DATE

        assert FIRST_APOD_DATE == datetime(1995, 6, 16)

    def test_date_format(self) -> None:
        from nasa_apod.services.apod.config import DATE_FORMAT

        assert DATE_FORMAT == "%Y-%m-%d"

    def test_request_timeout_is_positive(self) -> None:
        from nasa_apod.services.apod.config import REQUEST_TIMEOUT_SECONDS
...
class TestApiKeyLoading:
    ...
    def test_defaults_to_demo_key_when_env_unset(self, monkeypatch) -> None:
        ...
        import importlib
        import nasa_apod.services.apod.config as cfg
